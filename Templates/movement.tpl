...
$Ureturn = $database->getMovement(4,$village->wid,1);++$requse;
//$database->getMovement(4,$village->wid,0) ??
/* attack/raid on you! */
$Uattacked = $database->getMovement(3,$village->wid,1);++$requse;
/* on attack, raid | Unit send renfort other village | Unit send renfort in my village */
$Uattack = $database->getMovement(3,$village->wid,0);++$requse;


if( (count($Ureturn+$Uattacked+$Uattack)) > 0){echo'
<table id="movements" cellpadding="1" cellspacing="1">
	<thead><tr><th colspan="3">'.TROOP_MOVEMENTS.'</th></tr></thead>
	<tbody>';
}
...
if($aantal > 0){
	...
	echo '
		<tr>
			...
			<td><div class="mov"><span class="'.$aclass.'">'.$aantal.'&nbsp;'.$short.'</span></div><div class="dur_r">in&nbsp;<span id="timer'.$timer.'">'.$generator->getTimeFormat(min($NextArrival)-time()).'</span>&nbsp;'.HOURS.'</div></div></td>
		</tr>';
		$timer += 1;
}

/* attack/raid on you! */
$aantal = count($Uattacked);
$aantal2 = $Uattacked;
for($i=0;$i<$aantal;$i++){
