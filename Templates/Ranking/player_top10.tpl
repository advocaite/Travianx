...
<?php
    ...
    $result = mysql_query("SELECT * FROM ".TB_PREFIX."users WHERE access<".(INCLUDE_ADMIN?"10":"8")." AND tribe<=3 ORDER BY Rc DESC Limit 10");
...
<?php
    while($row = mysql_fetch_array($result2))
      ...
      echo "<td class=\"ra fc\">?&nbsp;</td>";
		if($row['username'] == $session->username) {
		echo "<td class=\"pla\">".$row['username']."</td>"; } else { echo "<td class=\"pla\"><a href='spieler.php?uid=".$row['id']."'>".$row['username']."</a></td>"; }      echo "<td class=\"val lc\">".$row['Rc']."</td>";
      echo "</tr>";
      }
?>
         </tbody>
