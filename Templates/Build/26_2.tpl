...
</table><p>Your villages have produced <b><?php echo $database->getUserField($session->uid, 'cp',0); ?></b> points in total. To found or conquer a new village you need <b><?php $mode = CP; $total = count($database->getProfileVillages($session->uid)); echo ${'cp'.$mode}[$total]; ?></b> points.</p>
</div>