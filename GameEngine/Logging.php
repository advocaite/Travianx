...
class Logging {
	...
	public function addIllegal($uid,$ref,$type){
		...
		if(LOG_ILLEGAL){
			$log = 'Attempted to ';
			switch($type){
				case 1:$log .= 'access village '.$ref;break;
			}
	...
	public function addMarketLog($wid,$type,$data){
		global $database;
		if(LOG_MARKET){
			if($type == 1)		{$log = 'Sent '.$data[0].','.$data[1].','.$data[2].','.$data[3].' to village '.$data[4];}
			elseif($type == 2)	{$log = 'Traded resource between '.$wid.' and '.$data[0].' market ref is '.$data[1];}
